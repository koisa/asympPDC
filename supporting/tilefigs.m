...
% ylen = fix(maxpos(4)/nrows) - 45; valor original % new tiled figure height
% ylen = fix(maxpos(4)/nrows) - 10; % new tiled figure height
...
%       else
%          figure(hands(pnum))
%          set(hands(pnum),'Position',[ xpos ypos xlen+22 ylen-30 ]); % move figure
%       end
%    end
% end
for iy = 1:nrows
   ypos = maxpos(4) - fix((iy)*maxpos(4)/nrows) + border; % figure location (row)
   for ix = 1:ncols
      xpos = fix((ix-1)*maxpos(3)/ncols + 1) + border;     % figure location (column)
      pnum = pnum+1;
      if (pnum>numfigs)
         break
      else
